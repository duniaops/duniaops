...
import { ROCKIMALS_TODAY_UI } from './rockimals-today.mjs';
import {
  ROCKIMALS_SMART_APP_BANNER,
  ROCKIMALS_STICKY_CTA_ASSETS,
  renderRockimalsStickyCta,
  rockimalsAppStoreUrl
...
function render(code) {
  ...
  const blogCard = featuredPost
    ? `<section aria-label="${text(l.navBlog)}"><a class="rk-blog-feature" href="${esc(featuredPost.canonicalPath)}"><img src="${esc(featuredPost.image)}" alt="" width="1200" height="630" loading="lazy" decoding="async"><span class="rk-blog-feature-copy"><span class="rk-blog-feature-eyebrow">${text(l.navBlog)}</span><strong>${text(featuredPost.title)}</strong><span class="rk-blog-feature-description">${text(featuredPost.description)}</span><span class="rk-blog-feature-cta">${text(blogReadLabel[code])} <span aria-hidden="true">→</span></span></span></a></section>`
    : '';
  const todayPath = `https://rockimals.duniaops.com${code === 'en' ? '/today' : `/${code}/today`}`;
  // The daily page's entry card. scripts/build-rockimals-today.mjs fills the
  // empty count span in dist once it has the day's list.
  const todayOrbs = ['mouse', 'tiger', 'whale']
    .map((hero) => `<img src="/assets/products/rockimals-heroes/loops/${hero}.webp" alt="" width="72" height="72" loading="lazy" decoding="async">`).join('');
  const todayCard = `<section aria-labelledby="rk-today-title"><a class="rk-today-card" href="${todayPath}"><span class="rk-today-orbs" aria-hidden="true">${todayOrbs}</span><span class="rk-today-copy"><span class="rk-today-eyebrow">${text(ROCKIMALS_TODAY_UI[code].kicker)}</span><strong id="rk-today-title">${text(ROCKIMALS_TODAY_UI[code].title)}</strong><span class="rk-today-lead"><span data-rockimals-today-count></span>${text(l.todayCardText)}</span></span><span class="rk-today-cta">${text(l.todayCardCta)} <span aria-hidden="true">→</span></span></a></section>`;
  // Both cards share the band that overlaps the hero's lower edge.
  const highlights = `<div class="rk-blog-highlight"><div class="rk-wrap rk-highlight-stack">${todayCard}${blogCard}</div></div>`;
...
<link rel="stylesheet" href="/css/rockimals-landing.css?v=20260924-today"><script src="/js/rockimals-landing.js?v=20260923-hero-loop" defer></script>${ROCKIMALS_STICKY_CTA_ASSETS}
...
${renderRockimalsStickyCta({ href: appStoreUrl, text: l.stickyText, storeLabel: l.appStore, closeLabel: l.stickyClose })}
</body></html>`;
}
